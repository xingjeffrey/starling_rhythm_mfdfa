## from Marvin, https://github.com/MarvinT/morphs/blob/master/morphs/utils/logistic.py
...
def max_ln_like(p, x, y):
    """max log likelihood for fitting the four parameter logistic.
    ...
    p_4pl = four_param_logistic(p)
    probs = p_4pl(x)
    return np.max(y * np.log(probs) + (1 - y) * np.log(1 - probs))
